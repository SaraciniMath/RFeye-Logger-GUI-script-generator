...
# pyuic5 RFNode_GUI_20_Scan.ui -o RFNode_GUI_20_Scan.py
...
from RFNode_GUI_21_Scan import Ui_Dialog
from RFNode_GUI_21_Timer import Ui_T2_Timer
from RFNode_GUI_21_Creditos import Ui_Creditos
from RFNode_GUI_21 import Ui_MainWindow
...
class InfoCreditos(QtWidgets.QDialog, Ui_Creditos):
    def __init__(self):
        super(InfoCreditos, self).__init__()
        self.setupUi(self)
        self.setWindowIcon(QtGui.QIcon(':/img1/if_logo-removebg-preview.ico'))
...
class TasksWindow(QtWidgets.QDialog, Ui_Dialog):
    def __init__(self):
        ...
        # self.setWindowIcon(_translate("MainWindow", "<html><head/><body><p><img src=\":/img1/if_logo-removebg-preview.ico\"/></p></body></html>"))
...
class PrincipalWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.window_creditos = InfoCreditos()
        self.window_creditos.setupUi(self.window_creditos)
        self.actionCr_ditos.triggered.connect(self.abre_creditos)
        self.actionDocumenta_o.triggered.connect(lambda: self.abre_url('docu'))
        self.actionReposit_rio.triggered.connect(lambda: self.abre_url('repo'))
        # self.setWindowIcon(QtGui.QIcon('resources/if_logo-removebg-preview.ico'))
        ...
        # validação numerica para intervalo
        ...
        self.scan_timer_radio.clicked.connect(lambda: self.radio_button_clicked(self.scan_timer_radio))
        self.scan_scan_radio.clicked.connect(lambda: self.radio_button_clicked(self.scan_scan_radio))

        self.updating = False
        self.scan_fcentral_sbox.valueChanged.connect(self.atualiza_spin_fcentral)
        self.scan_span_sbox.valueChanged.connect(self.atualiza_spin_fcentral)
        self.scan_fmin_sbox.valueChanged.connect(self.atualiza_spin_fintervalo)
        self.scan_fmax_sbox.valueChanged.connect(self.atualiza_spin_fintervalo)
        ...
        self.stream_inserir_vardir_bt.clicked.connect(lambda: self.inserir_variavel_stream('end'))
        self.stream_inserir_varname_bt.clicked.connect(lambda: self.inserir_variavel_stream('nome'))
    ...
    def abre_url(self, pagina):
        if pagina == 'repo':
            link = QUrl("https://github.com/SaraciniMath/RFeye-Logger-GUI-script-generator")
            QDesktopServices.openUrl(link)
        if pagina == 'docu':
            link = QUrl("https://github.com/SaraciniMath/RFeye-Logger-GUI-script-generator/tree/main/Manual")
            QDesktopServices.openUrl(link)

    def abre_creditos(self):
        self.window_creditos.exec_()
    ...
    def habilita_arquivo(self, enabled):
        if enabled:
            ...
            self.stream_vardir_cbox.setEnabled(True)
            self.stream_inserir_vardir_bt.setEnabled(True)
            self.stream_varname_cbox.setEnabled(True)
            self.stream_inserir_varname_bt.setEnabled(True)

    def radio_button_clicked(self, button):
        if button == self.scan_timer_radio:
            if self.selected_button == self.scan_timer_radio:
                self.scan_timer_radio.setChecked(True)
            else:
                self.selected_button = self.scan_timer_radio
                self.scan_scan_radio.setChecked(False)
        elif button == self.scan_scan_radio:
            if self.selected_button == self.scan_scan_radio:
                self.scan_scan_radio.setChecked(True)
            else:
                self.selected_button = self.scan_scan_radio
    ...
    def atualiza_spin_fcentral(self):
        if self.scan_fmin_sbox.value() >= self.scan_fmax_sbox.value():
            self.scan_fmin_sbox.setValue(self.scan_fmax_sbox.value()-1)
            return
        if self.scan_fmax_sbox.value() <= self.scan_fmin_sbox.value():
            self.scan_fmax_sbox.setValue(self.scan_fmin_sbox.value()-1)
            return
        if self.updating:
            return
        self.updating = True
        a = self.scan_fmin_sbox.value()
        b = self.scan_fmax_sbox.value()
        c = self.scan_fcentral_sbox.value()
        d = self.scan_span_sbox.value()
        a = c - d/2
        b = c + d/2
        self.scan_fmin_sbox.setValue(int(a))
        self.scan_fmax_sbox.setValue(int(b))
        self.updating = False

    def atualiza_spin_fintervalo(self):
        if self.scan_fmin_sbox.value() > self.scan_fmax_sbox.value() - 1:
            self.scan_fmin_sbox.setValue(self.scan_fmax_sbox.value())
            return
        if self.scan_fmax_sbox.value() < self.scan_fmin_sbox.value() + 1:
            self.scan_fmax_sbox.setValue(self.scan_fmin_sbox.value())
            return 
        if self.updating:
            return
        self.updating = True
        ...
        c = (b - a)/2 + a
        d = b - a
        self.scan_fcentral_sbox.setValue(int(c))
        self.scan_span_sbox.setValue(int(d))
        self.updating = False
    ...
    def inserir_variavel_stream(self, botao):
        if botao == 'end':
            # %(variavel1)s
            if not self.stream_vardir_cbox.currentText() == '':
                self.stream_end_edit.setText(self.stream_end_edit.text()
                                             + "%(" + self.stream_vardir_cbox.currentText() + ")s")
        if botao == 'nome':
            if not self.stream_varname_cbox.currentText() == '':
                self.stream_nome_edit.setText(self.stream_nome_edit.text()
                                              + "%(" + self.stream_varname_cbox.currentText() + ")s")
    ...
    def remover_variavel(self):
        ...
        for index_2 in range(self.conf_lista_lwid.count()):
            texto = str(self.conf_lista_lwid.item(index_2).text()).split(" ", 1)[0]
            self.stream_vardir_cbox.addItem(texto)
            self.stream_varname_cbox.addItem(texto)
    ...
    def inserir_variaveis(self):
        ...
        if self.conf_var_edit.text() in var_reservadas:
    ...
    def inserir_stream(self):
        # logica para validar o endereço
        # strings que começem sem /, que terminem com / ou pussuam sequência de / são inválidas
        text = self.stream_end_edit.text()
        match = re.search(r'^\/(?!\/)(?:(?!\/)[^\/]*\/)*(?!\/)[^\/]*(?<!/)$', text)
        # se o campo de endereço estiver vazio deve inserir um valor padrão
        if self.stream_end_edit.text() != '':
            if match:
                print("Valid input")
            else:
                print("Invalid input")
                ...
                msg.setInformativeText('Endereço deve obeder as seguintes regras:'
                                       '\n1. Começar com /'
                                       '\n2. Não conter / em sequência'
                                       '\n3. Não terminar com /')
                msg.setWindowTitle("Erro")
                msg.exec_()
                return
        if self.stream_id_edit.text() != '':
            ...
            if (self.stream_udp_radio.isChecked()) \
                    or self.stream_arq_radio.isChecked() or self.stream_http_radio.isChecked():
                if self.stream_id_edit.text() in self.list_id:
                    ...
                    if self.stream_http_radio.isChecked():
                        ...
                        if self.stream_end_edit.text() == '':
                            self.list_param[indice] = '/mnt/internal'
                            if self.stream_nome_edit.text() == '':
                                self.list_param[indice] += "/" + 'arquivo_medidas'
                            else:
                                self.list_param[indice] += "/" + self.stream_nome_edit.text()
                        else:
                            if self.stream_nome_edit.text() == '':
                                self.list_param[indice] = self.stream_end_edit.text() + "/" + 'arquivo_medidas'
                            else:
                    ...
                    if self.stream_http_radio.isChecked():
                        ...
                        if self.stream_end_edit.text() == '':
                            variavel += '/mnt/internal'
                            if self.stream_nome_edit.text() == '':
                                variavel += "/" + 'arquivo_medidas'
                                self.list_param.append('/mnt/internal' + "/" + 'arquivo_medidas')
                            else:
                                variavel += "/" + 'arquivo_medidas'
                                self.list_param.append('/mnt/internal' + "/" + self.stream_nome_edit.text())
                        else:
                            if self.stream_nome_edit.text() == '':
                                variavel += self.stream_end_edit.text() + "/" + 'arquivo_medidas'
                                self.list_param.append(self.stream_end_edit.text() + "/" + 'arquivo_medidas')
                            else:
    ...
    def inserir_execucao(self):
        if self.scan_timer_radio.isChecked():
            if self.scan_inter_edit.text() != '' and self.scan_nome_edit.text() != '':
                if int(self.scan_inter_edit.text()) <= 0:
                    msg = QtWidgets.QMessageBox()
                    msg.setWindowFlags(QtCore.Qt.CustomizeWindowHint |
                                       QtCore.Qt.WindowTitleHint
                                       )
                    msg.setIcon(QtWidgets.QMessageBox.Warning)
                    msg.setText("""O tempo de revisita deve ser maior que 0.""")
                    msg.setWindowTitle("Tempo de revisita inválido!")
                    msg.exec_()
                    return
                ...
                self.list_temp.append(str(int(self.scan_inter_edit.text())))
            ...
            if self.scan_inter_edit.text() != '' and self.scan_resol_sbox.text() != '' and self.scan_nome_edit.text() != '':
                ...
                self.list_interv.append(str(int(self.scan_inter_edit.text())))
